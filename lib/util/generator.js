...
generator.constructor = function (extendOptions) {
  ...
  var defaultValues = extendOptions.defaultValues

  var throwErrorInternal = function (error) {
    throw new Error('Model ' + extendOptions.name + ': ' + error)
  }

  // merge required and keys props
  if (_.isArray(extendOptions.required)) {
    required = _.union(required, extendOptions.required)
  }
  // exclude all default values from the required
  required = _.difference(required, Object.keys(defaultValues))
  ...
  return function (properties, options) {
    ...
    var self = this
    self = _.assign(self, defaultValues, properties)
  }
}
